...
awk -F ',' -v COL=3 -v column="RemGrefs" -f avgmax.awk $1
...
awk -F ',' -v COL=8 -v column="PhyWrs" -f avgmax.awk $1
...
./gnu_graph.sh $1 15 RouCMs 0 1 0
./gnu_graph.sh $1 16 Rourefs 0 1 0
./gnu_graph.sh $1 17 WDtmpq 0 1 0
...
then
	...
	./gnu_graphSnap.sh $1 15 "RouCMs Snapshot" 0 1 0
	./gnu_graphSnap.sh $1 16 "Rourefs Snapshot" 0 1 0
	./gnu_graphSnap.sh $1 17 "WDtmpq Snapshot" 0 1 0
	...
	if [ ! -d ${png_name}/png_m_snap ];
	then
		mkdir ${png_name}/png_m_snap
	fi	

	mv ${png_name}/*0.png ${png_name}/png_m_snap
