...
# Usage:    ./ddescend_P.sh -g -d ../ZCust/201503_Not_Trak/034_Roche/20150726/Customer_Info
...
do
	...
	then
		...
		if [ -z $snapshot ];
		then
			./dgsh -p -d "${InputFolder}/$j" -w "./perfmon_customer_templates/034_Roche_${j}" 2>/dev/null 
		else						
			./dgsh -p -s -d "${InputFolder}/$j" -w "./perfmon_customer_templates/034_Roche_${j}" 2>/dev/null 
		fi
		...
		if [ -z $snapshot ];
		then
			./dgsh -c -p -d "${InputFolder}/$j" -w "./perfmon_customer_templates/034_Roche_${j}" 2>/dev/null 
		else						
			./dgsh -c -p -s -d "${InputFolder}/$j" -w "./perfmon_customer_templates/034_Roche_${j}" 2>/dev/null 
		fi
